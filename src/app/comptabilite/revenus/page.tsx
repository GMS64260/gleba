...
/**
 * Page Revenus unifiés - Toutes les ventes de tous les modules
 */

import * as React from "react"
import Link from "next/link"
import { ArrowLeft, TrendingUp, RefreshCw, Filter, Sprout, TreeDeciduous, Bird } from "lucide-react"
...
const MODULE_ICONS: Record<string, React.ReactNode> = {
  ...
  autre: <TrendingUp className="h-4 w-4 text-blue-600" />,
}
...
export default function RevenusPage() {
  ...
  const [revenus, setRevenus] = React.useState<Revenue[]>([])
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/comptabilite/revenus?${params}`)
      if (response.ok) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        {stats && (
          <div className="grid gap-4 md:grid-cols-5 mb-6">
            ...
            <Card>
              ...
              <CardContent><p className="text-xl font-bold text-green-600">{formatEuro(stats.parModule.potager)}</p></CardContent>
            </Card>
            <Card>
              ...
              <CardContent><p className="text-xl font-bold text-lime-600">{formatEuro(stats.parModule.verger)}</p></CardContent>
            </Card>
            <Card>
              ...
              <CardContent><p className="text-xl font-bold text-amber-600">{formatEuro(stats.parModule.elevage)}</p></CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Autre</CardTitle></CardHeader>
              <CardContent><p className="text-xl font-bold text-blue-600">{formatEuro(stats.parModule.autre)}</p></CardContent>
            </Card>
          </div>
        )}
        ...
        {/* Liste */}
        <Card>
          <CardHeader><CardTitle>Détail des revenus ({revenus.length})</CardTitle></CardHeader>
                  ...
                  <TableBody>
                    {revenus.map((r) => (
                      <TableRow key={r.id}>
                        ...
                        <TableCell className="text-right font-bold text-green-600 whitespace-nowrap">{formatEuro(r.montant)}</TableCell>
                        ...
                        <TableCell><span className="text-xs text-muted-foreground">{r.source}</span></TableCell>
                      </TableRow>
                    ))}
                    {revenus.length === 0 && (
                      <TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">Aucun revenu enregistré</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              </div>
