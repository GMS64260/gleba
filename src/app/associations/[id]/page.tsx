...
/**
 * Page edition d'une association de plantes
 */

import * as React from "react"
import Link from "next/link"
import { useRouter, useParams } from "next/navigation"
import { ArrowLeft, Heart, Save, Plus, Trash2, Loader2 } from "lucide-react"
...
export default function EditAssociationPage() {
  ...
  const [association, setAssociation] = React.useState<AssociationData | null>(null)
  ...
  React.useEffect(() => {
    async function loadData() {
      try {
        const [associationRes, especesRes, famillesRes] = await Promise.all([
        ...
        if (!associationRes.ok) {
          throw new Error("Association non trouvee")
        }
  ...
  const onSubmit = async (data: UpdateAssociationInput) => {
