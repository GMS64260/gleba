...
export async function GET() {
  ...
  const [animaux, lots, exploitation] = await Promise.all([
    ...
    prisma.lotAnimaux.findMany({ where: { userId, statut: 'actif', quantiteActuelle: { gt: 0 } }, orderBy: [{ especeAnimaleId: 'asc' }, { nom: 'asc' }], include: { especeAnimale: { select: { nom: true } } } }),
  ...
  const buffer = await new Promise<Buffer>((resolve, reject) => {
    ...
    for (const lot of lots) { if (y > 540) { doc.addPage({ size: 'A4', layout: 'landscape', margin: 30 }); y = 40 } doc.text(`${lot.especeAnimale.nom} — ${lot.nom || `Lot #${lot.id}`} : ${lot.quantiteActuelle} animal(aux)`, 30, y); y += 15 }
    if (!animaux.length && !lots.length) doc.text('Aucun animal actif dans le cheptel.', 30, y)
    doc.end()
  })
